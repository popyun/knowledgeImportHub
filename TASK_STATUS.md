...
## 10. PP-StructureV3 ????????review-only ????2026-07-07 ???

????? PaddleOCR 3.7 ? PP-StructureV3 ????????????? ppstructurev3?????? B ??????????????????????????????????/??????????? [!warning] ??????????? review-only ?????????????????

??????????????????? paddle 3.x?? paddleocr 2.7.3?3.7 ???? OCR ??? PP-OCRv4 ?? v5???????? 33 ???????????????????????? PLAN ?????????????? paddleocr 2.7.3??? OCR ??????????PP-StructureV3 ???? venv ??????

??????
- processors/table_enhancer.py??? PPStructureV3Backend?????paddleocr 3.x ? import ? in-process???? config ppstructurev3_python ????? python ?????????? None ??????_extract_table_html ? V3 markdown ???? <table>?TableEnhancer._get_backend ?? ppstructurev3 ???enhance_regions ??? _v3_applicable ????????/??? + ?????????? MIN_ROWS=3/MIN_COLS=2/MAX_COLS=12/ROW_STAB_MIN=0.55?????????????
- processors/_ppstructurev3_worker.py??? 3.x ??? worker????PPStructureV3.predict??? {html} JSON?
- markdown_generator.py?_enhanced_block_for_region ? [!tip] ???? backend/engine ???? PP-Structure / PP-StructureV3?
- config.yaml?backend ?? ppstructurev3??? ppstructurev3_python??? python ??????? in-process ???????

???????
- pytest tests/ -q = 89 passed??? 13 ???????_v3_applicable ?????HTML ??????/in-process ?????????????????
- ?????enhance_on_low_quality:false ??????? 33 ????????? _regbaseline ???????? date ??MATCH 33 / DIFF 0?
- ????????121121???????? venv?D:/ocr_probe/venv_ppv3, paddleocr 3.7.0?????????? in-process ?????? 2.7.3?????? 5 ? 4 ???HTML 420 ?????? ~595s??? CPU ???

??????config.yaml: ocr.table_structure.enhance_on_low_quality: true + backend: "ppstructurev3" + ppstructurev3_python: "D:/ocr_probe/venv_ppv3/Scripts/python.exe"??????????????????

?????????????????121125 ???????????S_e ??????????????????????V3 ?? CPU 260~595s?????????????????? max_enhance_regions ?????

## 11. PP-StructureV3 ???? + ?????2026-07-08?

????????????????????????????????S_e ?????? plan-B ??? V3 ?????????? review-only ??????????

?????config.yaml ocr.table_structure??enhance_adopt(?? false) / enhance_adopt_margin / adopt_sb_max / adopt_se_min??????S_b < adopt_sb_max ? (S_e - S_b) >= margin ? S_e >= adopt_se_min??????? adopt??? compare?

??????table_enhancer._adopt_verdict() ????? verdict?adopt/compare????????? compare?markdown_generator?verdict==adopt ?? V3 ??? plan-B ??????????? [!note] ?????????????? + review-only [!tip]???? _match_enhanced_item ???????

???????_v3calib ???? 13 ??????6 ???? / 7 ? / 10 ??????????S_b ?? 0.43~0.997?????????????????
- S_e ?????????6 ????????????? V3 ??????? plan-B???121125 r02 S_e=0.886 ????????121128 r04 gain +0.19 ???????????121134 r00 gain -0.11 ????
- ?????????? 210~350px?V3 ??????????????????
- ????? 3B ? V3 ???????????????????????????????????enhance_adopt_margin=0.30 / adopt_sb_max=0.55 / adopt_se_min=0.90??? 0.15/0.62/0.80 ???????? 121125 r02?????????????????????????

????????? enhance_adopt=false ? 33 ??????? _regbaseline ????? MATCH 33 / DIFF 0?pytest tests/ -q = 97 passed??? 8 ???????????? ad655ca???????????
